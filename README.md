# AI 学习记录

从零基础起步，往 **大模型训练与推理系统** 方向走的学习记录。

四川大学 · 计算机科学与技术

> 规则：笔记自己整理；**代码保证能跑通再提交**；不放任何个人材料（有提交前自动扫描兜底）。

---

## 知识地图（学到哪了）

图例：✅ 能自己讲清 + 动手做过 · 🟡 正在学 · ⬜ 还没开始

| 模块 | 状态 | 已经掌握的内容 | 笔记 |
|---|---|---|---|
| 训练与推理基础 | 🟡 | KV Cache；Prefill/Decode 两种负载（算力 vs 带宽）；MoE 稀疏与路由；多模态 Encoder；**梯度检查点＝拿计算换显存** | [笔记①](notes/20260921_KV-Cache.md) ｜ [笔记②](notes/20260922_推理负载与梯度检查点.md) |
| 数据挖掘与信息检索 | 🟡 | Web Mining 三块（内容/结构/使用挖掘）+ 暑期课的备考路线 | [notes/20260921_Web-Mining（网络数据挖掘）.md](notes/20260921_Web-Mining（网络数据挖掘）.md) |
| 论文阅读 | 🟡 | Agent SFT 的 loss mask 与 RL 探索（ActObs 论文）+ 前置知识清单与三遍读法 | [notes/20260921_论文_Dont-Mask-the-Environment.md](notes/20260921_论文_Dont-Mask-the-Environment.md) |
| 前沿模型动态 | 🟡 | Jev / System One 模型（不生成文本、只输出类型化决策 + 校准概率） | [notes/20260921_Jev与System-One模型.md](notes/20260921_Jev与System-One模型.md) |
| **深度学习地基**（前置知识） | 🟡 | 导数与链式法则（最关键）、矩阵乘法与形状、函数斜率直觉；含 3 道自测题 | [notes/20260922_深度学习前置知识清单.md](notes/20260922_深度学习前置知识清单.md) |
| 训练与推理原理 | 🟡 | 梯度下降 · 反向传播链式法则 · 激活函数 · 梯度消失/爆炸 · CNN 卷积 | [notes/20260922_推理负载与梯度检查点.md](notes/20260922_推理负载与梯度检查点.md) |
| 环境与工具链 | ✅ | WSL、Python 环境、Git 工作流、提交前隐私扫描、**云 GPU 上手（AutoDL）** | [00-setup/20260921_环境清单.md](00-setup/20260921_环境清单.md) ｜ [00-setup/20260923_云GPU上手清单.md](00-setup/20260923_云GPU上手清单.md) |
| PyTorch 与深度学习基础 | ⬜ | 张量 / 自动求导 / 训练循环 → 自己写 MNIST | — |
| Transformer 与大模型 | ⬜ | 注意力机制 → nanoGPT → LoRA 微调 | — |
| 数据库与 AI4DB | ⬜ | 存储与索引 / 查询优化 → GPTuner | — |
| 分布式训练 | ⬜ | 数据并行 / ZeRO / 通信 → mLoRA 复现 | — |

## 学过的概念（速查）

一条概念一页笔记，30 秒能读完，一律先讲生活类比再上术语。

- **KV Cache** —— 生成下一个字时，前面算过的 Key/Value 直接复用，不必重算；代价是显存随上下文线性增长。→ [笔记](notes/20260921_KV-Cache.md)
- **Web Mining（网络数据挖掘）** —— 从网页和用户行为里挖信息：内容挖掘（抓正文）、结构挖掘（超链接算重要性）、使用挖掘（点击流做推荐）。→ [笔记](notes/20260921_Web-Mining（网络数据挖掘）.md)
- **ActObs：把观测也纳入 SFT 损失** —— 标准 Agent SFT 只对动作 token 算损失、mask 掉环境观测；把观测也作为预测目标（不加数据/参数/算法改动）能让后续 GRPO 的 pass@k 更高、熵保持更好。→ [笔记 + 前置知识清单](notes/20260921_论文_Dont-Mask-the-Environment.md)

## 环境

| 项 | 值 |
|---|---|
| 本地 | WSL Ubuntu 20.04，无 GPU |
| 训练 | 云 GPU（按小时租，跑完即停） |
| Python | 3.11 + PyTorch（版本记录在 00-setup） |
| 版本管理 | Git + GitHub（提交前自动隐私扫描） |

## 学习路线

六个支柱的**技能树**（含依赖图、每个技能点的通关标准、当前站位、可并行的两条线）→ [学习路线.md](学习路线.md)

## 目录

```
00-setup/           环境配置与踩坑记录
01-pytorch-basics/  PyTorch 练习脚本（每个文件独立可运行）
02-transformer/     注意力 / nanoGPT / LoRA 实验
03-database/        数据库与 AI4DB 笔记、SQL 练习
04-systems/         分布式训练复现笔记
notes/              小概念卡片（一个概念一页）
```

## 记录规范

**笔记五段式**（照这个写，别人一眼能看出你会不会做事）：

1. 这次要解决什么（一句话）
2. 怎么跑的（完整命令 + 数据规模 + 硬件）
3. 结果（数字 / 图 / 报错原文）
4. 踩的坑与解法（最重要，别省略）
5. 还没搞懂的（诚实留白，比硬装懂好）

**代码规矩**：命名 `日期_主题.py`；文件开头写清任务、依赖、运行命令；能跑通再提交，运行输出粘进对应笔记。
